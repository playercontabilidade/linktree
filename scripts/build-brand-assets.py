...
Entradas  (companies/player/assets/):
    logo_grupo_player.svg   lockup "Grupo Player" para fundo claro
    favicon.svg             símbolo "play" isolado

Ambas foram removidas do repo e as duas etapas pulam sozinhas quando a origem
falta — pegue o arquivo no histórico do Git se precisar regerar. O lockup
"Grupo Player" saiu de uso quando o hero passou a usar o do site; quem cuida
desse é scripts/build-site-logo.py.
...
def build_dark_logo() -> None:
    ...
    if not src_path.exists():
        print(f"  pulando: {src_path.name} não está no repo (veja o histórico)")
        return
