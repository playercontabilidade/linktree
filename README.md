...
| Tipografia | Inter e Fraunces, self-hosted (woff2 variável subsetado) |
...
| Tipografia | `--font-sans`, `--font-serif` | Inter no corpo, Fraunces italic na assinatura; tracking negativo nos títulos |
...
| Arquivo | Uso | Peso |
|---|---|---|
| `logo_player_on_dark.png` | Lockup do topo no tema escuro. Lettering claro, símbolo dourado. | 15 KB |
| `logo_player_on_light.png` | Lockup do topo no tema claro. Mesmo símbolo, lettering em `#27313d`. | 15 KB |
| `mark_player.png` | Só o símbolo, recortado. Ícone da linha "Site — Player Contabilidade". | 6 KB |
| `logo_grupo_player_light.png` | Arte de origem das três acima, como `playercontabilidade.com` a serve. Não é carregada pela página. | 16 KB |
| `favicon.ico`, `favicon-32.png`, `favicon-180.png` | Ícone do navegador e da tela de início do iOS. | 43 KB |

### Por que os SVGs precisavam de tratamento

> Os dois SVGs do lockup "Grupo Player" saíram do repo quando o hero passou a
> usar o lockup do site (242 KB sem referência). Estão no histórico do Git, e
> `build-brand-assets.py` pula as etapas que dependem deles. A seção fica como
> registro: se algum dia chegar arte nova da agência no mesmo formato, é este
> o tratamento que ela precisa.

Os SVGs entregues eram **híbridos**: no `logo_grupo_player.svg` o wordmark são
...
| Logo invisível | A variante do lockup não bate com o tema. `.brand__logo` troca o `background-image` por `[data-theme]`; se o seletor não casar, sobra o lettering claro no fundo claro |
